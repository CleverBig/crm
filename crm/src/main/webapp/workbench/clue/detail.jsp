...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#aname").keydown(function (event) {
			if(event.keyCode==13){
				$.ajax({
					url: "workbench/clue/showBundActivityList.do",
					data: {
					...
					success: function(data){
						...
						$.each(data,function(i,v){
							$("#searchActivityBody").append(
								'<tr>'+
								'<td><input type="checkbox" name="select" value="'+v.id+'"/></td>'+
								'<td>'+v.name+'</td>'+
								'<td>'+v.startDate+'</td>'+
								'<td>'+v.endDate+'</td>'+
								'<td>'+v.owner+'</td>'+
		...
		$("#bundBtn").click(function(){
			var selects = $("input[name=select]:checked");
			if(selects.length==0){
				alert("请至少选择一个市场活动");
			}else{
				var param = "cid=${clue.id}&";
				for (var i = 0; i < selects.length; i++) {
					param += "aid="+$(selects[i]).val();
				...
				$.each(data,function(i,v){
					$("#activityBody").append(
						'<tr>'+
							...
							'<td>'+v.endDate+'</td>'+
							'<td>'+v.owner+'</td>'+
							'<td><a href="javascript:void(0);" onclick="unbund(\''+v.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>'+
						'</tr>'
					)
				})
			}
